...
define([
  ...
  const d = new o.GeographicProjection();
  function S(t, a, e, o, r, s, i) {
    const c = Math.cos(a),
      g = o * c,
      l = e * c,
      S = Math.sin(a),
    ...
    (h = n.Matrix2.multiplyByVector(m, h, h)),
      ...
      (s -= 1),
      (i -= 1);
    const X = (t = d.unproject(h, t)).latitude,
      Y = X + s * M,
      f = X - g * i,
      p = X - g * i + s * M,
      x = Math.max(X, Y, f, p),
    ...
    return {
      north: x,
      south: R,
      east: Math.max(G, y, O, b),
      west: Math.min(G, y, O, b),
      granYCos: g,
      granYSin: w,
      granXCos: l,
      granXSin: M,
  ...
  (g.computeOptions = function (t, a, e, o, s, i, c) {
    ...
    if (0 !== o) {
      e -= o;
      const t = n.Rectangle.northwest(P, c),
        a = S(t, e, x, R, 0, f, p);
      (W.stGranYCos = a.granYCos),
        (W.stGranXCos = a.granXCos),
        (W.stGranYSin = a.granYSin),
        (W.stGranXSin = a.granXSin),
        (W.stNwCorner = t),
        (W.stWest = a.west),
        (W.stSouth = a.south);
    }
