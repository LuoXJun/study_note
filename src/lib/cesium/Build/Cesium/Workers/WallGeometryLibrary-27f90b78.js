...
define([
  ...
  (a.computePositions = function (e, a, p, d, y, f) {
    ...
    const P = a.length,
      A = P - 2;
    let C, w;
    const b = o.CesiumMath.chordLength(y, e.maximumRadius),
      v = u;
    if (((v.minDistance = b), (v.ellipsoid = e), f)) {
      let e,
        t = 0;
      for (e = 0; e < P - 1; e++)
        t += r.PolylinePipeline.numberOfPoints(a[e], a[e + 1], b) + 1;
      (C = new Float64Array(3 * t)), (w = new Float64Array(3 * t));
      const i = g,
        n = c;
      (v.positions = i), (v.height = n);
      let o = 0;
      for (e = 0; e < P - 1; e++) {
        (i[0] = a[e]), (i[1] = a[e + 1]), (n[0] = p[e]), (n[1] = p[e + 1]);
        const t = r.PolylinePipeline.generateArc(v);
        C.set(t, o),
          (n[0] = d[e]),
          (n[1] = d[e + 1]),
          w.set(r.PolylinePipeline.generateArc(v), o),
          (o += t.length);
      }
    } else
      (v.positions = a),
        (v.height = p),
        (C = new Float64Array(r.PolylinePipeline.generateArc(v))),
        (v.height = d),
        (w = new Float64Array(r.PolylinePipeline.generateArc(v)));
