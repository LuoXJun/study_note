...
define([
  ...
  './Plane-e916220d'
], function (e, t, n, i, s, r, h, o, u, d, p, a, l, f, c, g, m) {
  'use strict';
  const x = {
      ...
      computeLineSegmentLineSegmentIntersection: function (
        ...
        const a = (d - o) * (s - e) - (u - h) * (r - t);
        if (0 === a) return;
        const l = ((u - h) * (t - o) - (d - o) * (e - h)) / a,
          f = ((s - e) * (t - o) - (r - t) * (e - h)) / a;
  ...
  function k(e, t, n, s, r, h, o, u, d) {
    ...
    let p = 0,
      a = 0;
    for (; a < o.length; ) a = X[p++].initializeFromClipResult(o, a, u);
    for (let r = 0; r < p; ++r) {
      const o = X[r];
      if (o.isIndexed())
  ...
  return (
    ...
    d(function (e, i) {
      const h = e.isEastChild,
        ...
        m = z,
        U = M;
      (c.length = 0), (g.length = 0), (m.length = 0), (U.length = 0);
      const F = V;
      F.length = 0;
      const P = {},
        D = e.vertices;
      ...
      let Be = xe.east;
      const ye = xe.west;
      for (Be < ye && (Be += o.CesiumMath.TWO_PI), $ = 0; $ < c.length; ++$)
        (te = Math.round(c[$])),
          te <= p
            ? (de.push($), (te = 0))
            : te >= a
      ...
      const Ie = t.BoundingSphere.fromVertices(ge, n.Cartesian3.ZERO, 3, E),
        Ae = u.OrientedBoundingBox.fromRectangle(xe, fe, ce, me, H),
        ...
        ve = ce - fe,
        Te = new Uint16Array(c.length + g.length + m.length);
      for ($ = 0; $ < c.length; ++$) Te[$] = c[$];
      let ze = c.length;
      for ($ = 0; $ < g.length; ++$) Te[ze + $] = g[$];
      for (ze += g.length, $ = 0; $ < m.length; ++$)
        Te[ze + $] = (w * (m[$] - fe)) / ve;
      const Ve = r.IndexDatatype.createTypedArray(c.length, F);
      let Me;
      if (K) {
